...
class MuseTrialProcessor:
            ...
            for method in parsing_methods:
                try:
                    ...
                    if 'Cond' in trial_info.columns:
                        break
                    # If not, see if we can find a similar column
    ...
    def apply_ica(self):
        ...
        try:
            ...
            # Make sure we have the correct exclusions set
            ...
            # Use our comprehensive before/after visualization 
    ...
    def _detect_eog_artifacts(self, data_to_use):
        try:
            # Make sure ICA has been fit
            if not hasattr(self, 'ica') or self.ica is None:
                self.logger.warning("ICA must be fit before detecting EOG artifacts")
                return False
                    
            # MUSE has no dedicated EOG channel, but we can use frontal electrodes
            if 'AF7' in self.CHANNELS['EEG'] and 'AF8' in self.CHANNELS['EEG']:
                self.logger.info("Using frontal electrodes for EOG detection")
                
                # Create virtual EOG as difference between AF7 and AF8
                eeg_data = data_to_use.get_data()
                af7_idx = data_to_use.ch_names.index('AF7')
                af8_idx = data_to_use.ch_names.index('AF8')
                virtual_eog = eeg_data[af7_idx] - eeg_data[af8_idx]
                
                # Create temporary Raw object with just this EOG channel
                info = mne.create_info(['EOG'], data_to_use.info['sfreq'], ['eog'])
                virtual_eog_raw = mne.io.RawArray(virtual_eog.reshape(1, -1), info)
                
                # Use MNE's EOG detection function
                self.logger.info("Finding EOG events using MNE's find_eog_events function")
                ...
                if len(eog_events) > 0:
                    self.logger.info(f"Found {len(eog_events)} EOG events")
                    
                    # Create epochs around EOG events
                    eog_epochs = mne.Epochs(
                        data_to_use,
                        eog_events,
                        event_id=998,  # Arbitrary event ID
                        tmin=self.CONFIG['ICA']['blink_tmin'],
                        tmax=self.CONFIG['ICA']['blink_tmax'],
                        baseline=self.CONFIG['TRIAL_WINDOW']['BASELINE'],
                        preload=True
                    )
                    
                    # Log ICA component information
                    self.logger.info(f"ICA components shape: {self.ica.get_components().shape}")
                    
                    # Calculate correlation with virtual EOG directly
                    self.logger.info("Using correlation-based EOG detection")
                    
                    # Get sources
                    sources = self.ica.get_sources(data_to_use).get_data()
                    
                    # Calculate correlation with virtual EOG - this is the most reliable method
                    n_components = self.ica.n_components_
                    eog_scores = np.zeros(n_components)
                    for i in range(n_components):
                        # Ensure we don't go out of bounds
                        if i < sources.shape[0]:
                            corr = np.corrcoef(sources[i], virtual_eog)[0, 1]
                            eog_scores[i] = np.abs(corr)
                            # Safely extract the scalar value
                            score_val = float(eog_scores[i])
                            self.logger.info(f"Component {i} correlation with EOG: {score_val:.3f}")
                        else:
                            self.logger.warning(f"Component {i} out of bounds (total sources: {sources.shape[0]})")
                            eog_scores[i] = 0.0
                    
                    # Use threshold to identify EOG components
                    valid_scores = eog_scores[:sources.shape[0]]  # Only use valid components
                    if len(valid_scores) > 0:
                        threshold_val = max(0.3, np.percentile(valid_scores, 75))
                        threshold = float(threshold_val)
                        eog_indices = np.where(valid_scores > threshold)[0]
                        
                        # If no components found, take the highest scoring one if above minimum threshold
                        if len(eog_indices) == 0 and np.max(valid_scores) > 0.2:
                            max_idx = np.argmax(valid_scores)
                            if max_idx < sources.shape[0]:  # Safety check
                                eog_indices = [int(max_idx)]
                                self.logger.info(f"Selecting highest correlation component {eog_indices[0]}")
                        
                        # Set exclude property
                        self.ica.exclude = list(map(int, eog_indices))  # Convert all indices to int
                        self.logger.info(f"Marked {len(self.ica.exclude)} components for exclusion")
                        
                        # Visualize
                        self._visualize_component_scores(eog_scores)
                        
                        return True
                    else:
                        self.logger.warning("No valid scores found for EOG detection")
                        return False
                else:
                    self.logger.warning("No EOG events detected")
                    
                    # Fall back to pure correlation-based detection
                    self.logger.info("Using correlation-based EOG detection as fallback")
                    
                    # Get sources
                    sources = self.ica.get_sources(data_to_use).get_data()
                    
                    # Calculate correlation with virtual EOG
                    n_components = min(self.ica.n_components_, sources.shape[0])
                    eog_scores = np.zeros(n_components)
                    for i in range(n_components):
                        corr = np.corrcoef(sources[i], virtual_eog)[0, 1]
                        eog_scores[i] = np.abs(corr)
                        # Safely extract scalar value
                        self.logger.info(f"Component {i} correlation with EOG: {float(eog_scores[i]):.3f}")
                    
                    # Use threshold to identify EOG components
                    threshold_val = max(0.3, np.percentile(eog_scores, 75))
                    threshold = float(threshold_val)
                    eog_indices = np.where(eog_scores > threshold)[0]
                    
                    # If no components found, take the highest scoring one
                    if len(eog_indices) == 0 and np.max(eog_scores) > 0.2:
                        max_idx = np.argmax(eog_scores)
                        eog_indices = [int(max_idx)]
                        self.logger.info(f"Selecting highest correlation component {eog_indices[0]}")
                ...
                self.logger.warning("Frontal electrodes (AF7, AF8) not available for EOG detection")
                return False
                
        except Exception as e:
            self.logger.error(f"Error detecting EOG artifacts: {e}")
            self.logger.error(traceback.format_exc())
            return False
        
    def _calculate_blink_scores(self, sources, eog_data, peaks):
        """Calculate correlation scores between components and blink pattern."""
        blink_scores = []
        
        if len(peaks) > 0:
            try:
                # Use a longer window for blink detection to capture the entire blink
                window_size = int(0.3 * self.CONFIG['SAMPLING_RATE'])  # 300ms window (increased from 200ms)
                
                # Use the average of multiple blinks as template if possible
                blink_segments = []
                for peak_idx in peaks[:min(5, len(peaks))]:  # Use up to 5 peaks
                    # Extract blink pattern safely
                    start_idx = max(0, peak_idx - window_size)
                    end_idx = min(len(eog_data[0]), peak_idx + window_size)
                    
                    if end_idx - start_idx > window_size:  # Only use if window is large enough
                        blink_segments.append(eog_data[0, start_idx:end_idx])
                
                # Average the blink segments if we found any
                if blink_segments:
                    # Standardize length to shortest segment
                    min_length = min(len(segment) for segment in blink_segments)
                    standardized_segments = [segment[:min_length] for segment in blink_segments]
                    blink_pattern = np.mean(standardized_segments, axis=0)
                else:
                    # Fallback to single blink
                    start_idx = max(0, peaks[0] - window_size)
                    end_idx = min(len(eog_data[0]), peaks[0] + window_size)
                    blink_pattern = eog_data[0, start_idx:end_idx]
                
                # Lower correlation threshold to capture more potential blink components
                min_correlation = 0.3  # Lower threshold to include more potential components
                
                # Calculate correlation for each source
                for i, src in enumerate(sources):
                    src_avg = src.mean(axis=0)
                    src_center = len(src_avg) // 2
                    
                    # Get matching segments
                    src_start = max(0, src_center - len(blink_pattern)//2)
                    src_end = min(len(src_avg), src_start + len(blink_pattern))
                    src_segment = src_avg[src_start:src_end]
                    
                    # Calculate correlation only if segments are long enough
                    if len(src_segment) >= 2 and len(blink_pattern) >= 2:
                        min_len = min(len(src_segment), len(blink_pattern))
                        corr = np.corrcoef(src_segment[:min_len], blink_pattern[:min_len])[0, 1]
                        
                        # Use absolute correlation but with a higher weight for positive correlation
                        # This assumes blinks are usually positively correlated with frontal activity
                        if np.isnan(corr):
                            score = 0
                        else:
                            score = abs(corr) * (1.2 if corr > 0 else 0.8)
                        
                        blink_scores.append(score)
                        self.logger.info(f"Component {i} correlation score: {score:.3f}")
                    else:
                        blink_scores.append(0)
                        
                # Force exclusion of at least one component if any score is above minimum threshold
                if max(blink_scores) > min_correlation and not any(score > 0.6 for score in blink_scores):
                    self.logger.info(f"Forcing exclusion of highest correlation component (score: {max(blink_scores):.3f})")
                    
            except Exception as e:
                self.logger.error(f"Error calculating blink scores: {e}")
                self.logger.error(traceback.format_exc())
                return np.zeros(len(sources))
                    
        return np.array(blink_scores)
    ...
    def visualize_ica_components_with_timeseries(self):
        ...
        try:
            # Check if we already have a figure open - this prevents duplicates
            if hasattr(self, 'ica_components_fig') and plt.fignum_exists(self.ica_components_fig.number):
            ...
            for idx in range(n_components):
                ...
                # Plot the topography with parameters that work with your MNE version
                try:
                    ...
                    try:
                        ...
                        # Just leave an empty plot if we can't make the topography
    ...
    def enhanced_plot_sources(self):
            """
            Custom wrapper for ica.plot_sources that adds topographies on the left side.
            This modifies MNE's plot_sources functionality to include topography maps.
            """
            if not hasattr(self, 'ica') or self.ica is None:
                self.logger.warning("ICA must be fit before visualization")
                return
            ...
            try:
                # Call our custom visualization function but don't close existing figures
                fig = self.visualize_ica_components_with_timeseries()
                
                # Store reference to prevent garbage collection
                self.ica_sources_fig = fig
                
                # Return the figure without calling plt.show() yet
                return fig
            
            except Exception as e:
                self.logger.error(f"Error in enhanced plot_sources: {e}")
                self.logger.error(traceback.format_exc())
                return None
    ...
    def _mark_excluded_components(self, figures):
        """Mark excluded components with red indicator in component plots."""
        try:
            ...
            for fig in figures:
                ...
                # For each component subplot
                for ax in axes:
                    title = ax.get_title()
                    if not title or 'ICA' not in title:
                        continue
                        
                    # Extract component number from title (format is usually "ICA000")
                    match = re.search(r'ICA(\d+)', title)
                    if not match:
                        continue
                    ...
                    if comp_num in self.ica.exclude:
                        # Add red box around excluded components
                        ...
                        ax.set_title(f"{title} (excluded)", color='red', fontweight='bold')
        except Exception as e:
            self.logger.warning(f"Error marking excluded components: {e}")
    ...
    def visualize_ica_components(self):
        ...
        try:
            # Create our custom visualization
            fig = self.visualize_ica_components_with_timeseries()
    ...
    def _visualize_ica_correlation(self, ax):
        """Visualize correlation between ICA components."""
        try:
            # Use EEG data only
            eeg_picks = mne.pick_types(self.filtered_data.info, eeg=True, meg=False, exclude=[])
            data_for_components = self.filtered_data.get_data(picks=eeg_picks)
            
            # Get sources from ICA
            comp_data = np.dot(self.ica.unmixing_matrix_, data_for_components)
            
            # Calculate correlation matrix
            corr_matrix = np.corrcoef(comp_data)
            ...
            # Set labels
            labels = [f'IC{i+1}' for i in range(min(comp_data.shape[0], self.ica.n_components_))]
            ax.set_xticks(range(len(labels)))
            ax.set_yticks(range(len(labels)))
            ax.set_xticklabels(labels)
            ax.set_yticklabels(labels)
            ax.set_title('Component Cross-correlation')
            
        except Exception as e:
            self.logger.error(f"Error visualizing ICA correlation: {e}")
            self.logger.error(traceback.format_exc())
            
    def _visualize_ica_weights(self, ax):
        """Visualize channel weights for each ICA component."""
        try:
            # Get EEG channel indices
            eeg_picks = mne.pick_types(self.filtered_data.info, eeg=True, meg=False, exclude=[])
            
            # Extract weights for EEG channels only
            eeg_channel_names = [self.filtered_data.ch_names[i] for i in eeg_picks]
            components = self.ica.get_components()
            weights = components[eeg_picks, :self.ica.n_components_]
            ...
            # Set labels
            ax.set_yticks(range(len(eeg_channel_names)))
            ax.set_yticklabels(eeg_channel_names)
            
            labels = [f'IC{i+1}' for i in range(self.ica.n_components_)]
            ax.set_xticks(range(self.ica.n_components_))
            ax.set_xticklabels(labels)
            ...
            ax.set_title('Component Weights per Channel')
            
        except Exception as e:
            self.logger.error(f"Error visualizing ICA weights: {e}")
            self.logger.error(traceback.format_exc())
            
    def _visualize_ica_time_series(self, ax):
        """Visualize ICA component time series."""
        try:
            # Get only EEG data to ensure dimension match
            eeg_picks = mne.pick_types(self.filtered_data.info, eeg=True, meg=False, exclude=[])
            data_for_components = self.filtered_data.get_data(picks=eeg_picks)
            ...
            # Calculate times
            times = np.arange(comp_data.shape[1]) / self.CONFIG['SAMPLING_RATE']
            
            # Plot each component
            for i in range(min(comp_data.shape[0], self.ica.n_components_)):
                color = 'red' if i in self.ica.exclude else 'blue'
                ax.plot(times, comp_data[i], color=color, alpha=0.7,
                    label=f'IC{i+1}{" (excluded)" if i in self.ica.exclude else ""}')
            ...
            self.logger.error(f"Error visualizing ICA time series: {e}")
            self.logger.error(traceback.format_exc())
    ...
    def _update_ica_time_display(self):
        ...
        for idx, ax in enumerate(time_axes):
            ...
            # Style the plot - NO y-axis label to avoid duplication
    ...
    def keep_plots_open(self):
        """Safeguard method preventing plots from closing prematurely."""
        # Collect all figures from various attributes 
        figures = []
        
        # Check for standard figure attributes
        for attr_name in dir(self):
            if attr_name.endswith('_fig') and hasattr(self, attr_name):
                fig = getattr(self, attr_name)
                if isinstance(fig, plt.Figure):
                    figures.append(fig)
            elif attr_name == 'figures' and hasattr(self, 'figures'):
                if isinstance(self.figures, list):
                    figures.extend(self.figures)
        
        # If no figures found but we have ICA, create the visualization
        if not figures and hasattr(self, 'ica') and self.ica is not None:
            comp_fig = self.visualize_ica_components_with_timeseries()
            if comp_fig:
                figures.append(comp_fig)
        
        # Show all figures
        if figures:
            self.logger.info(f"Displaying {len(figures)} figures. Close all windows to exit.")
            
            # Make sure all figures are visible
            for fig in figures:
            ...
            self.logger.warning("No figures available to display.")
            
            # Create at least one figure if none exists
            self.logger.info("Creating basic visualization...")
            if hasattr(self, 'ica') and self.ica is not None:
                self.visualize_ica_components_with_timeseries()
                plt.show(block=True)
    ...
    def toggle_filters(self, label):
        ...
        if label == 'EEG Filters':
            ...
            # We've already handled the update in toggle_ica, so return early
            return
    ...
    def update_condition_average(self, condition):
        """Update the average plot for a specific condition."""
        self.current_avg_condition = condition
        self.ax_avg.clear()
        self.ax_pd.clear()
        ...
        try:
            # Check available conditions in trials
            available_conditions = list(self.trials.event_id.keys()) if hasattr(self.trials, 'event_id') else []
            self.logger.info(f"Available conditions in trials: {available_conditions}")
            
            # Define colors for conditions if TRIAL_TYPES not in CONFIG
            condition_colors = {
                'highpos': '#1f77b4',  # blue
            ...
            # Handle grand average specially
            if condition == 'Grand Average' and hasattr(self.trials, 'event_id'):
                # Plot all available trials regardless of condition
                all_trials = self.trials
                self._plot_condition_data(all_trials, times, 'All Trials', color='black', line_style='-')
                
                # Try to plot individual conditions if they exist
                for cond_name in available_conditions:
                    try:
                        cond_trials = self.trials[cond_name]
                        if len(cond_trials) > 0:
                            color = condition_colors.get(cond_name, None)
                            self._plot_condition_data(cond_trials, times, cond_name, color=color)
                    except Exception as e:
                        self.logger.warning(f"Could not plot condition {cond_name}: {str(e)}")
            ...
            # Handle specific condition
                ...
                if len(cond_trials) > 0:
                    ...
                    self._plot_condition_data(cond_trials, times, condition, color=color)
                else:
                    self.logger.warning(f"No trials found for condition {condition}")
                    self._plot_empty_condition(times, condition)
                    
            # Fallback: plot all trials if condition not found
            else:
                self.logger.warning(f"Condition '{condition}' not found - plotting all trials instead")
                all_trials = self.trials
                title = f"All Trials ({len(all_trials)})"
                self._plot_condition_data(all_trials, times, title, color='black', line_style='-')
                
            # Add title with trial count
            if condition == 'Grand Average':
                title = f"Grand Average - {len(self.trials)} total trials"
            elif condition in available_conditions:
                title = f"{condition} - {len(self.trials[condition])} trials"
            else:
                title = f"All Trials - {len(self.trials)} trials"
                
            self.ax_avg.set_title(title)
            self._style_average_plots()
            
        except Exception as e:
            self.logger.error(f"Error updating condition average: {str(e)}")
            self.logger.error(traceback.format_exc())
    ...
    def _plot_condition_data(self, epochs, times, condition, color=None, line_style='-'):
        """Plot condition data with unique styling for each."""
        # Get color based on condition if not provided
        if color is None:
            ...
            if condition in condition_colors:
                ...
                # Default colors for unknown conditions
                colors = ['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd']
                color = colors[hash(condition) % len(colors)]
        
        # Get photodiode data
        if 'AUX_L' in epochs.ch_names:
            pd_idx = epochs.ch_names.index('AUX_L')
        ...
        for ch in selected_channels:
            if ch in epochs.ch_names:
                ...
                if len(electrode_data) > 0 and not np.all(np.isnan(electrode_data)):
                    ch_data.append(electrode_data)
        
        if not ch_data:
            self.logger.warning("No valid electrode data found")
            return
            
        # Average across selected channels first, then across trials
        all_avg_data = []
        for trial_idx in range(len(ch_data[0])):
            # Get data for this trial across all selected channels
            trial_ch_data = [ch[trial_idx] for ch in ch_data if trial_idx < len(ch)]
            # Average across channels for this trial
            if trial_ch_data:
                ch_avg = np.nanmean(trial_ch_data, axis=0)
                all_avg_data.append(ch_avg)
        
        if all_avg_data:
            # Convert to array and calculate average/SEM across trials
            all_avg_data = np.array(all_avg_data)
            grand_avg = np.nanmean(all_avg_data, axis=0)
            grand_stderr = sem(all_avg_data, axis=0, nan_policy='omit')
            
            # Plot ERP
            self.ax_avg.plot(times, grand_avg, color=color,
                        label=f"{'/'.join(selected_channels)} ({condition})",
                        linewidth=self.CONFIG['PLOT']['STYLES']['line_width'],
                        linestyle=line_style)
    ...
    def _style_average_plots(self):
        ...
        stim_start = 0
        stim_duration = 600  # Default 600ms stimulus
        if hasattr(self, 'avg_stim_duration'):
            stim_duration = self.avg_stim_duration * 1000  # Convert to ms
            
        for ax in [self.ax_avg, self.ax_pd]:
            # Clear any previous stimulus windows
            for patch in ax.patches:
                if hasattr(patch, '_stim_window'):
                    patch.remove()
            ...
            patch._stim_window = True  # Mark this patch as stimulus window
