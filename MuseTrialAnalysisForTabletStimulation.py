...
class MuseTrialProcessor:
        ...
        self.CONFIG = {
            ...
            'PHOTODIODE': {
                'DETECTION': {
                    ...
                    'CHANNEL': 'AUX_R',          # Photodiode channel name
    ...
    def get_condition_name(self, cond_code):
        ...
        return condition_map.get(cond_code, 'unknown')
        
    def load_trial_info(self, filepath=None):
        """Load trial information from stimulus log file (.txt/.dat)."""
        if filepath is None:
            # If no filepath provided, try to find a TXT/DAT file in the same directory
            if self.input_filepath:
                base_dir = os.path.dirname(self.input_filepath)
                txt_files = [f for f in os.listdir(base_dir) if f.endswith('.txt') or f.endswith('.log')]
                if len(txt_files) > 0:
                    filepath = os.path.join(base_dir, txt_files[0])
                    self.logger.info(f"Auto-detected stimulus log file: {filepath}")
                else:
                    # Ask user to select a file
                    root = tk.Tk()
                    root.withdraw()
                    filepath = filedialog.askopenfilename(
                        title="Select stimulus log file (.txt/.dat)",
                        filetypes=[("Text files", "*.txt *.dat"), ("All files", "*.*")]
                    )
                    root.destroy()
                    
        if not filepath:
            self.logger.warning("No stimulus log file selected")
            return False
            
        self.trial_info_filepath = filepath
        
        try:
            # First try to read file as the new stimulus log format
            with open(filepath, 'r') as f:
                content = f.readlines()
            
            # Check if the file contains the new format header with Timestamp
            if len(content) > 0 and "Timestamp" in content[0]:
                self.logger.info("Detected new stimulus log format (Timestamp,BlockTrial,StimFile,...)")
                
                # Parse each line to extract trial information
                trial_data = []
                for i, line in enumerate(content[1:]):  # Skip header
                    try:
                        parts = line.strip().split(',')
                        if len(parts) >= 3:  # Ensure at least timestamp, block/trial, and stim file
                            stim_file = parts[2].strip() if len(parts) > 2 else ""
                            
                            # Skip lines that don't have a proper stimulus file (may be header or comments)
                            if not stim_file or not ('_' in stim_file and '.' in stim_file):
                                continue
                                
                            # Extract condition from stim file (e.g., "highpos_29.jpg" -> "highpos")
                            condition = None
                            if stim_file:
                                condition_match = re.match(r'([a-zA-Z]+)_\d+\.jpg', stim_file)
                                if condition_match:
                                    condition = condition_match.group(1).lower()
                            
                            # Map condition to a numeric code
                            cond_code = 0
                            if condition == 'highpos':
                                cond_code = 1
                            elif condition == 'neutral':
                                cond_code = 2
                            elif condition == 'lowpos':
                                cond_code = 3
                            elif condition == 'highneg':
                                cond_code = 4
                            elif condition == 'lowneg':
                                cond_code = 5
                            
                            # Only add entries that have a valid condition
                            if cond_code > 0:
                                trial_data.append({
                                    'Trial': i + 1,
                                    'Cond': cond_code,
                                    'StimFile': stim_file,
                                    'Timestamp': float(parts[0]) if parts[0].replace('.', '', 1).isdigit() else 0,
                                    'ConditionName': condition
                                })
                    except Exception as e:
                        self.logger.warning(f"Error parsing line {i+1}: {str(e)}")
                
                # Create DataFrame from parsed data
                self.trial_info = pd.DataFrame(trial_data)
                
                # Log condition counts
                condition_counts = self.trial_info['Cond'].value_counts()
                for cond, count in condition_counts.items():
                    condition_name = self.get_condition_name(cond)
                    self.logger.info(f"Condition {cond} ({condition_name}): {count} trials")
                
                # Log total number of trials found
                self.logger.info(f"Found {len(self.trial_info)} total trials in log file")
                ...
                # Fall back to original method for handling DAT files
                self.logger.info("Trying to read as original DAT file format")
    ...
    def detect_photodiode_events(self, data):
        """
        Detect photodiode upwave signals with robust fallback options.
        Specifically tailored to detect patterns in the MUSE AUX channels.
        """
        ...
        min_val = np.min(pd_data)
        max_val = np.max(pd_data)
        std_val = np.std(pd_data)
        self.logger.info(f"Photodiode channel range: {min_val:.1f} to {max_val:.1f}")
        self.logger.info(f"Standard deviation: {std_val:.1f}")
        
        # DIRECT APPROACH: Look for exactly 1650.0 values (prioritize this method)
        self.logger.info("Using direct detection method for 1650.0 μV values")
        exact_high_values = np.where(pd_data == 1650.0)[0]
        
        if len(exact_high_values) > 0:
            self.logger.info(f"Found {len(exact_high_values)} samples with exact 1650.0 μV value")
            
            # Group consecutive samples into events
            events_start = []
            i = 0
            while i < len(exact_high_values):
                # Start of a new event
                start_idx = exact_high_values[i]
                j = i + 1
                
                # Find all consecutive samples
                while j < len(exact_high_values) and exact_high_values[j] == exact_high_values[j-1] + 1:
                    j += 1
                    
                # Calculate end index and duration
                end_idx = exact_high_values[j-1]
                duration = (end_idx - start_idx + 1) / SF
                
                # Only consider sequences of sufficient duration
                if duration >= 0.01:  # At least 10ms
                    events_start.append(start_idx)
                    self.logger.info(f"Found 1650.0 μV event at {start_idx/SF:.3f}s, duration: {duration:.3f}s")
                    
                # Move to next potential event
                i = j
            
            if len(events_start) > 0:
                self.logger.info(f"Identified {len(events_start)} potential photodiode events")
                found_events = events_start
        else:
            self.logger.warning("No samples with exact 1650.0 μV found")
        
        # If direct approach failed, try threshold approach
        if len(found_events) == 0:
            self.logger.info("Trying threshold approach to detect high photodiode values")
            
            # Constants for detecting value above high threshold
            HIGH_LEVEL_THRESHOLD = 1600    # Minimum value for upper level trigger
            MIN_DURATION = int(0.01 * SF)  # Minimum 10ms duration (adapt for sample rate)
            
            # Find all values above high threshold
            high_indices = np.where(pd_data > HIGH_LEVEL_THRESHOLD)[0]
            
            if len(high_indices) > 0:
                self.logger.info(f"Found {len(high_indices)} samples with values above {HIGH_LEVEL_THRESHOLD} μV")
                ...
                while i < len(high_indices):
                    ...
                    # Find all consecutive samples
                    while j < len(high_indices) and high_indices[j] == high_indices[j-1] + 1:
                        j += 1
                        
                    # Calculate end index and duration
                    end_idx = high_indices[j-1]
                    duration = (end_idx - start_idx + 1) / SF
                    
                    # Only consider sequences of sufficient duration
                    if (end_idx - start_idx + 1) >= MIN_DURATION:
                        events_start.append(start_idx)
                        self.logger.info(f"Found high-value event at {start_idx/SF:.3f}s, duration: {duration:.3f}s")
                ...
                self.logger.warning(f"No samples with values above {HIGH_LEVEL_THRESHOLD} μV")
        
        # Final fallback: Look for large differences
        if len(found_events) == 0:
            self.logger.info("Using edge detection approach as last resort")
            
            # Calculate first-order differences
            diff_data = np.diff(pd_data)
            
            # Look for large positive jumps (at least 200 μV)
            large_jumps = np.where(diff_data > 200)[0]
            
            if len(large_jumps) > 0:
                self.logger.info(f"Found {len(large_jumps)} large jumps in the photodiode signal")
                
                # To identify distinct events, find jumps that are separated by a minimum interval
                min_interval = int(0.5 * SF)  # Minimum 500ms between events
                
                if len(large_jumps) > 0:
                    # Start with the first jump
                    distinct_jumps = [large_jumps[0]]
                    
                    # Add all jumps that are separated by at least min_interval
                    for jump in large_jumps[1:]:
                        if jump - distinct_jumps[-1] >= min_interval:
                            distinct_jumps.append(jump)
                    
                    found_events = distinct_jumps
                    self.logger.info(f"Identified {len(found_events)} distinct events from large jumps")
            else:
                self.logger.warning("No large jumps found in the photodiode signal")
        
        # Filter out events that are too close together (keep only first of each trigger)
        if len(found_events) > 1:
            min_separation = int(0.4 * SF)  # Minimum 400ms between trials
            filtered_events = [found_events[0]]
            
            for e in found_events[1:]:
                if e - filtered_events[-1] >= min_separation:
                    filtered_events.append(e)
                    
            if len(filtered_events) < len(found_events):
                self.logger.info(f"Filtered out {len(found_events) - len(filtered_events)} closely spaced events")
                found_events = filtered_events
                    
        self.logger.info(f"Found {len(found_events)} valid photodiode events")
        
        # Add detection rate metrics
        if len(found_events) > 1:
            intervals = np.diff(found_events) / SF
            self.logger.info(f"Average interval between events: {np.mean(intervals):.2f}s (std: {np.std(intervals):.2f}s)")
            if len(intervals) > 0:
                self.logger.info(f"Interval range: {np.min(intervals):.2f}s to {np.max(intervals):.2f}s")
        
        return np.array(found_events)
    
    def visualize_photodiode_detection(self):
        """Visualize photodiode signal with detected upwave events."""
        ...
        try:
            # Get photodiode data
            pd_idx = self.raw_data.ch_names.index(self.CONFIG['PHOTODIODE']['DETECTION']['CHANNEL'])
            pd_data = self.raw_data.get_data()[pd_idx]
            
            # Get detected events
            events = self.detect_photodiode_events(self.raw_data)
            
            # Create a time array
            time = np.arange(len(pd_data)) / self.CONFIG['SAMPLING_RATE']
            
            # Create plot
            fig, ax = plt.subplots(figsize=(15, 6))
            ax.plot(time, pd_data, 'b-', linewidth=1, alpha=0.7)
            
            # Mark detected events
            for evt in events:
                evt_time = evt / self.CONFIG['SAMPLING_RATE']
                ax.axvline(x=evt_time, color='r', linestyle='--', alpha=0.8)
                ax.plot(evt_time, pd_data[evt], 'ro', markersize=8)
                
            # Mark threshold
            ax.axhline(y=self.CONFIG['PHOTODIODE']['DETECTION']['THRESHOLD'], 
                    color='g', linestyle='-', alpha=0.5,
                    label=f"Threshold ({self.CONFIG['PHOTODIODE']['DETECTION']['THRESHOLD']} uV)")
            
            # Add labels and title
            ax.set_xlabel('Time (s)')
            ax.set_ylabel('Amplitude (uV)')
            ax.set_title(f'Photodiode Signal with Detected Events (n={len(events)})')
            ax.legend()
            
            # Show grid
            ax.grid(True, alpha=0.3)
            
            # If events exist, zoom to the first few events for better visibility
            if len(events) > 0:
                # Show 2 seconds before first event and 5 seconds total
                first_evt_time = events[0] / self.CONFIG['SAMPLING_RATE']
                ax.set_xlim(max(0, first_evt_time - 2), max(5, first_evt_time + 3))
                
                # Set y limits to focus on the signal range
                min_val = np.percentile(pd_data, 5)
                max_val = np.percentile(pd_data, 95)
            ...
            self.logger.error(f"Error visualizing photodiode detection: {str(e)}")
            self.logger.error(traceback.format_exc())
            return None
    
    def create_trials(self):
        """
        Create epochs aligned to photodiode events with trial condition mapping.
        Ignores POSIX timestamps and relies solely on trial sequence from log file.
        """
        self.logger.info("Creating trials from photodiode signals...")
        ...
        try:
            ...
            self.logger.info("Trial #  |  Sample Index  |  Time (s)")
            self.logger.info("----------------------------------------")
            ...
            for i, event_sample in enumerate(raw_events):
                ...
                self.logger.info(f"{i+1:6d}  |  {int(event_sample):12d}  |  {event_time_seconds:9.3f}")
            ...
            # Create basic events array - always works even if event_id mapping fails
            events = np.zeros((len(raw_events), 3), dtype=int)
            events[:, 0] = raw_events  # Sample indices
            events[:, 2] = 1          # Default event ID
            
            # Create basic trials first with no condition mapping
            data_to_use = self.filtered_data if hasattr(self, 'filtered_data') and self.CONFIG['FILTERS']['EEG']['enabled'] else self.raw_data
            ...
            if hasattr(self, 'trial_info') and self.trial_info is not None:
                try:
                    ...
                    # Decide how many events to use - minimum of detected events and trial info rows
                    num_detected_trials = len(raw_events)
                    num_log_trials = len(self.trial_info)
                    
                    self.logger.info(f"Detected {num_detected_trials} photodiode events, log contains {num_log_trials} trials")
                    
                    # Use the minimum to avoid indexing errors
                    num_trials_to_use = min(num_detected_trials, num_log_trials)
                    self.logger.info(f"Using first {num_trials_to_use} trials for condition mapping")
                    
                    # Create new events with condition codes - IMPORTANT: Order follows the log file
                    condition_events = events.copy()
                    
                    # Take the first num_trials_to_use entries from trial_info for condition codes
                    for i in range(num_trials_to_use):
                        condition_events[i, 2] = self.trial_info['Cond'].values[i]
                    
                    # Create inverse mapping from code to string
                    code_to_name = {v: k for k, v in self.trials_event_id.items()}
                    
                    # Create event_id from the condition codes present in events
                    present_codes = np.unique(condition_events[:num_trials_to_use, 2])
                    event_id_map = {code_to_name.get(code, f'cond_{code}'): code for code in present_codes}
                    
                    self.logger.info(f"Using event_id mapping: {event_id_map}")
                    
                    # Create new epochs with condition information
                    try:
                        self.trials = mne.Epochs(
                            data_to_use,
                            condition_events[:num_trials_to_use],  # Use only the matched trials
                        ...
                        # Store the event_id for reference
                        self.trials_event_id = event_id_map
                        
                        # Log successful condition application
                        self.logger.info(f"Successfully applied condition information to {len(self.trials)} trials")
                        
                        # Log condition distribution in the epochs
                        for cond_name, cond_id in event_id_map.items():
                            try:
                                cond_count = len(self.trials[cond_name])
                                self.logger.info(f"  Condition '{cond_name}' (id:{cond_id}): {cond_count} trials")
                            except:
                                self.logger.warning(f"  Could not count trials for condition '{cond_name}'")
                        
                    except Exception as e:
                        self.logger.error(f"Error creating epochs with conditions: {str(e)}")
                        self.logger.error(traceback.format_exc())
                        self.logger.info("Falling back to basic trial structure without conditions")
                    
                    # Log condition for each trial
                    self.logger.info("========== TRIALS WITH CONDITIONS ==========")
                    self.logger.info("Trial #  |  Condition  |  Sample Index  |  Time (s)")
                    self.logger.info("--------------------------------------------------")

                    for i, timestamp_data in enumerate(self.trial_timestamps):
                        if i < num_trials_to_use:
                            condition = self.trial_info['Cond'].values[i]
                            condition_name = self.condition_map.get(condition, f"Unknown ({condition})")
                            sample_index = timestamp_data['sample_index']
                            time_seconds = timestamp_data['time_seconds']
                            
                            self.logger.info(f"{i+1:6d}  |  {condition_name:10s}  |  {sample_index:12d}  |  {time_seconds:9.3f}")
    ...
    def save_trials_to_edf(self):
        """Save individual trials as EDF files and create a combined EDF file with improved error handling."""
        try:
            ...
            if len(self.trials) == 0:
                self.logger.warning("No valid trials to save (empty Epochs object)")
                return
            ...
            if hasattr(self, 'trials') and hasattr(self.trials, 'events') and hasattr(self.trials, 'event_id'):
                ...
                try:
                    ...
                    self.logger.warning(f"Could not get condition labels from events: {str(e)}")
            ...
            for i, trial_data in enumerate(self.trials.get_data()):
                ...
                for ch_idx, ch_name in enumerate(self.trials.ch_names):
                    ...
                    if np.isnan(signal_data).any():
                        self.logger.warning(f"NaN values found in trial {i+1}, channel {ch_name}")
                    ...
                    # Create truncated labels for combined file
                    # Format: ChName_T# (e.g., AUX_L_T10)
                    short_cond = condition_labels[i][:3].lower() if i < len(condition_labels) else 'unk'  # first 3 chars of condition
                ...
                # Skip saving individual trial if there's a problem
                if not signals:
                    self.logger.warning(f"No valid signals for trial {i+1}, skipping")
                    skip_count += 1
                    continue
                ...
                try:
                    ...
                    cond_label = condition_labels[i] if i < len(condition_labels) else 'unk'
                    ...
                    if trial_count % 10 == 0:
                        self.logger.info(f"Saved {trial_count} trials so far")
                except Exception as e:
                    self.logger.error(f"Error saving trial {i+1}: {str(e)}")
                    skip_count += 1
            
            self.logger.info(f"Successfully saved {trial_count} individual trial EDF files")
            if skip_count > 0:
                self.logger.warning(f"Skipped {skip_count} trials due to errors")
            
            # Create combined EDF file for all trials if signals found
            if combined_signals:
                try:
                ...
                self.logger.warning("No combined EDF file created as no valid signals were found")
            ...
            with open(metadata_file, 'w') as f:
                ...
                # Add trial sample indices
                f.write("\nTrial Sample Indices:\n")
                f.write("Trial #  |  Sample Index  |  Time (s)\n")
                f.write("----------------------------------\n")
                
                for ts_data in self.trial_timestamps:
                    f.write(f"{ts_data['trial_num']:6d}  |  {ts_data['sample_index']:12d}  |  {ts_data['time_seconds']:9.3f}\n")
                    
            self.logger.info(f"Successfully saved trial data to {save_dir}")
        ...
        try:
            ...
            if self.CONFIG['FILTERS']['AUX']['enabled'] and len(self.CHANNELS['AUX']) > 0:
                ...
                if self.CONFIG['FILTERS']['AUX']['lowpass'] is not None:
                    self.filtered_data.filter(
                        l_freq=self.CONFIG['FILTERS']['AUX']['lowpass'],
                        h_freq=None,
                        picks=self.CHANNELS['AUX'],
                        method=self.CONFIG['FILTERS']['AUX']['method'],
                        phase=self.CONFIG['FILTERS']['AUX']['phase'],
    ...
    def visualize_ica_components_with_timeseries(self):
        ...
        try:
            # Check if there is a figure already open - this prevents duplicates
            if hasattr(self, 'ica_components_fig') and plt.fignum_exists(self.ica_components_fig.number):
            ...
            for idx in range(n_components):
                ...
                try:
                    ...
                    try:
                        ...
                        # Leave an empty plot if topography unavailable 
    ...
    def _visualize_component_scores(self, scores):
        ...
        try:
            ...
            # Case-check for no components scores
            if len(scores) == 0:
    ...
    def _plot_condition_data(self, epochs, times, condition, color=None, line_style='-'):
        # Define distinct colors for individual electrodes and pairs
        electrode_colors = {
